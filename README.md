...
[Nyman fixed-modulus third-step refinement](results/nyman-fixed-modulus-refinement-v1.json),
[Nyman certified fourth rebased step](results/nyman-rebased-fourth-step-v1.json),
...
The separate `N=256`, `K=16` direction has now crossed that finite-certification
boundary after exact coarse rounding. Its [complete-tail proof](docs/nyman-balanced-full-tail-v1.md)
uses a 16-bit old vector, a 9-bit shell and multiplier, and the resulting
18-bit convolved correction supported through index 8,192. The prefix through
`T=2^26` and the full analytic tail give

```text
0.0001420236276423717967... < complete direct gain
                                    < 0.0003677819654619422937...,
```

so the exact public claim `gain > 7/50000` holds. The checked artifact payload
SHA-256 is
`47adcf95031f0e51203c56332beafcfd049374b3446fbe212d4ae7a474958377`.
Its verifier replays at 384 bits with half-sized binary64 blocks. This proves
...
#### Genuine nesting and the frozen-prefix obstruction

The [nested-chain certificate](docs/nyman-nested-chain-v1.md) replaces the
cross-sectional comparison by an actual update sequence:

```text
p1 = p0 + a1: complete gain > 1/150,  support 8 -> 1,024
p2 = p1 + a2: complete gain > 1/9000, support 1,024 -> 16,384
telescoped complete gain > 61/9000
```

The verifier reconstructs `p1` before it builds the second shell, and the
stored `p1` updated-vector commitment must equal the second step's old-vector
commitment. The second direction uses exact `K=8` coefficients, outer scale
`1/4`, a prefix through `2^25`, and an independent 448-bit replay.

The same audit also closes this append-only architecture as an all-scale RH
route. If a correction is supported above the existing core and has
`sum(a_n/n)=0`, then it vanishes identically on that core interval. Every
later residual therefore retains a fixed positive piece of the seed residual.
No finite seed can make that whole piece zero: the interval equations would
force `p_n=-mu(n)`, while a Bertrand-prime modulo argument proves
`sum_(n<=Q) mu(n)/n != 0`. Thus every strictly append-only
harmonic-balanced shell chain has a positive norm floor.

For the exact `N=8` seed used here, the simpler quantitative floor is

```text
P1 = sum(p_n/n) = 241057/27525120
energy >= P1^2 = 58108477249/757632231014400
                  ~= 0.0000766974725603.
```

This is a route rejection, not a disproof of RH. The surviving construction
must modify earlier coefficients through overlap, rebasing, or residualized
Schur-complement reoptimization.

#### Rebased Schur contraction with arbitrary harmonic slope

The [rebased certificate](docs/nyman-rebased-schur-v1.md) takes that surviving
route for one explicit step. Starting from the exact sparse `p1`, it derives a
new rounded ideal shell but then rebuilds `p2` from eight direct coordinates,
eight dilates of the first shell, and eight dilates of the new shell. The old
coordinates are reoptimized, so `p2-p1` is not an append-only
harmonic-balanced correction and the frozen-prefix theorem does not apply.

For an arbitrary finite vector `p`, put

```text
P = sum(p_n/n),  q_M = 1 + sum p_n floor(M/n).
```

Including `(0,1)`, the exact prefix through interval `T` is

```text
(T+1)P^2 + sum_(M<=T) q_M^2/[M(M+1)] - 2P L_T,
```

where Abel summation compresses `L_T` to finitely many `log` and `log Gamma`
terms. Centering the periodic residual gives exact mean square `rho`; the
Montgomery--Vaughan large sieve and Abel summation then give the absolute tail
...
At `T=2^17`, the exact rational tail upper bound for the 915-term `p2` is
about `0.000346903036160492`. Direct Arb evaluation of the 55-term `p1` and
the complete upper bound for `p2` produce a certified gain-lower computation
`[0.0003176470 +/- 5.96e-11]`, whose lower endpoint is greater than `1/5000`.
...
The optimizer is not evidence. This artifact alone contains no fourth step or
all-scale continuation; the two separately bound certificates below extend
the finite record without changing the global verdict.

#### Fixed-modulus refinement of the same third step

The
[fixed-modulus certificate](docs/nyman-fixed-modulus-refinement-v1.md)
keeps the published `p2`, `p3`, and cutoff `T=2^23` unchanged. For the active
denominator set `D`, fixed `L`, and `m_d=gcd(d,L)`, it uses
...
The Farey gap `gcd(d,e)/(de)` makes this a valid local reciprocal-spacing
charge. At `L=1` the formula exactly recovers the active-`Q` third-step
constant. For the frozen `p3`, choosing `L=840` groups 17,157 active
denominators into 32 gcd classes and reduces the discrepancy constant from
about `2.146613456e9` to `1.816711318e9`. The complete tail upper bound falls
to about `0.0000269295240130`, and the exact enclosure subtraction proves

```text
E(p2) - E(p3) > 0.0000573056 > 1/17500.
```

The artifact payload SHA-256 is
`de69623dee4550796471850d62bf78ec3162fdec34620526f21d0ac578f35f92`.
This is a sharper finite proof for the same vectors, not a new coefficient
construction or an arbitrary-scale theorem.

#### Certified fourth rebased step

The
[fourth-step certificate](docs/nyman-rebased-fourth-step-v1.md) reconstructs
the exact published `p3`, derives a balanced `2^-9` shell on
`32769..65536`, and freezes a thirty-three-entry `2^-16` proposal. Its basis
reoptimizes the earlier direct and shell coordinates while retaining only the
first multiplier of the fourth shell. The resulting exact `p4` has 47,345
nonzero coefficients, is supported through 65,536, and changes 13,537 old
`p3` coefficients.

At `T=2^26`, the ordinary denominator-weighted constant
`C_loc=(3/2)Q sigma` gives a complete `p4` tail upper bound of about
`0.00000523133452109`. The old-prefix-lower versus
new-prefix-plus-tail-upper comparison proves

```text
E(p3) - E(p4) > 0.0000105439 > 1/100000.
```

Generation uses 512-bit Arb with block size 1,048,576; replay changes to
640-bit Arb and block size 250,003. The artifact payload SHA-256 is
`a807f63928869c6451fa397edcc294ff6b3553b69a54099e8053722bd2b594b0`.

Algebraically composing the two separately certified finite inequalities
gives

```text
E(p2) - E(p4) > 1/17500 + 1/100000 = 47/700000.
```
...
.\.venv\Scripts\python.exe tools\generate_nyman_nested_chain_certificate.py `
